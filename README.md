...
サイボウズ株式会社のグループウェア Garoon を使いやすくするための Chrome 拡張機能（Manifest V3）です。日表示・週表示に現在時刻を示すラインをオーバーレイ表示し、予定の簡易登録ダイアログに ToDo を追加するタブを差し込みます。
...
- **現在時刻ライン**: 日表示・週表示で現在時刻を示す赤いラインを表示します。オプションページから ON/OFF を切り替え可能です。
- **予定の簡易登録ダイアログに ToDo タブを追加**: 予定の簡易登録ダイアログ上部にタブバーを差し込み、「予定」と「ToDo」を切り替えられるようにします。「ToDo」タブから ToDo 名・締切日・メモを入力して、その場で Garoon に ToDo を登録できます。オプションページから ON/OFF を切り替え可能です。

※ これらの機能は Garoon の公開 API ではなく画面の DOM 構造や内部エンドポイントに依存して実装されているため、Garoon 側のアップデートにより動作しなくなる可能性があります。
